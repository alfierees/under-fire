...
## Site structure (as of v3 overhaul — 2026-04-17; expanded 2026-07-04)
- `index.html` — hero (Iron Dome canvas + hero-bg.jpg + "UNDER FIRE" title) + card grid: **Graphs** (4), **Maps** (3), **Patterns** (2), **Stats & Reference** (3). Hero counters carry real static floor text ("160,000+"), not "0" — crawlers that don't run JS read them.
...
- `arsenal.html` — "The Arsenal" weapon-systems reference. **Generated page**: edit `data/processed/arsenal.json` (curated) or `scripts/_arsenal_template.html`, then `python3 scripts/build_arsenal_page.py .` — never hand-edit arsenal.html's static sections.
- `data.html` — open-data page: schema + downloads for every dataset, provenance/caveats, citation block, Dataset JSON-LD.
...
- `area_monthly.json` — per-area counts for every calendar month (contiguous first→last). Feeds `timelapse.html`.
- `operations.json` — per-operation/campaign stats + daily series for `compare.html`. Windows/editorial text: `OPERATIONS` in `scripts/process_data.py`.
- `arsenal.json` — **curated editorial** weapon-systems reference (specs, salvo history, defenses, dated events; sources per entry). NOT pipeline-derived — edit by hand, then rerun `scripts/build_arsenal_page.py`.

**Pipeline also writes to root:** `sitemap.xml` and `llms.txt` are regenerated by `process_data.py` on every run (fresh lastmod + key figures for AI crawlers). Don't hand-edit — change `SITEMAP_PAGES` / `write_llms_txt()` in the script.

**Favicon:** `images/favicon.png` is 96×96 (Google search-result favicons require a multiple of 48px) plus a root `favicon.ico`. Keep both sizes if the logo art changes.
