...
## 2026-07-04 session — favicon, GEO, four new pages

- **Favicon fixed for Google search**: `images/favicon.png` upscaled 64→96px (Google requires a multiple of 48px), `sizes="96x96"` on every page, root `favicon.ico` added. Still needed from Alfie: request homepage re-crawl in Search Console; expect days-to-weeks for the icon to appear in results.
- **GEO (AI-crawler) package**: `llms.txt` (pipeline-generated with live key figures), `sitemap.xml` now pipeline-generated (fresh lastmod, new pages), hero counters carry real static floors instead of "0", JSON-LD on all new pages, workflows commit llms.txt+sitemap.xml.
- **New pages**: `timelapse.html` (monthly choropleth animation, `area_monthly.json`), `compare.html` (9 operations incl. the 2026 Total War, `operations.json`), `arsenal.html` (28 weapon systems + 7 defenses + escalation timeline, curated `arsenal.json`, page generated by `scripts/build_arsenal_page.py`), `data.html` (open-data documentation).
- **Pipeline**: `process_data.py` gained `area_monthly` + `operations` generators and root `sitemap.xml`/`llms.txt` writers. `OPERATIONS` windows/editorial text live in the script (same rule as story chapters).
- Fixed a long-standing timeline.html console error (negative rect width from a rAF timestamp race).

## 2026-07-04 session, round 3 — records fix + duel v2
- **records.html blank cards fixed**: `days_under_attack` + `major_attack_days` were never emitted by `build_records()` (page guarded silently). Now computed from TRUE per-day counts read from the master (not the estimated `daily_counts.json`). Card descriptions corrected to match reality (one-in-three days, one-in-eleven major days — the old "four in five"/"one in six" text was wrong).
- **Duel v2**: selectable defense (Auto / Iron Dome / David's Sling / Arrow / THAAD / Iron Beam) with per-class engagement envelopes — wrong layer = "cannot engage" warning and everything leaks; three batteries drawn, interceptors launch from them; Iron Beam renders as an instant laser at ~$5/shot; saturation curve softened (×80 ≈ reported ~86–90% rates, ×1 nearly always a kill).
- **Future (owner wishlist)**: more accurate weapon models (commissioned 2D sprites or 3D) to replace the schematic SVG silhouettes; a map-based launch view (arc from Tehran/Gaza/Sanaa over real geography, reusing arcs.html patterns); mixed-volley mode (rockets + drones + BMs in one salvo, the April-2024 pattern).

## 2026-07-04 session, round 4 — map view + code review + hardcoded audit
- **Arsenal duel gained a Map view** (now the default; Side view is a toggle): real Leaflet geography, the projectile arcs from its actual front (Gaza/Lebanon/Yemen/Iran) to a real Israeli city, the map auto-frames to the threat's range (Qassam → tight on the Gaza border; Shahed → the whole Red Sea approach), three interceptor batteries ring the city. Requires Leaflet (added to `_arsenal_template.html`). Falls back to Side-only if Leaflet is absent.
- **Ran a full multi-angle code review + hardcoded-values audit. Fixes applied:**
  - `arsenal-sim.js`: map arcs no longer detach from the basemap on a mid-flight window resize (`trackResize:false` + deferred reframe); launch stagger is now real-time (was a hardcoded `-=16.7`/frame that desynced on 120Hz/30fps); canvas dims cached per fire (no per-frame layout); `unitCost` read from the memoized `tally.sim.cost`.
  - `process_data.py`: `operations()` skips an ongoing op whose start postdates the data (no negative-length window); `llms.txt` percentages guarded against a zero-total dataset.
  - `generate_extra_aggregates.py`: `major_attack_days.one_in_every` guarded against zero major-days.
  - `timelapse.html`: the "biggest month / regions ever hit" narrative figures are now **computed client-side** from `area_monthly.json` (were hardcoded prose that would go stale) — static text is the fetch-failure fallback.
